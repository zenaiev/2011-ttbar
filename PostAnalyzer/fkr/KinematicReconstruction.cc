...
KinematicReconstruction::KinematicReconstruction(const TString& fileInput, const int minNumberOfBtags, const bool preferBtags, const bool massLoop):
//era_(era),
...
h_mbl_w_(0)
{
    ...
    if(!massLoop_) this->loadData(fileInput);

    std::cout<<"=== Finishing preparation of kinematic reconstruction\n\n";

    std::cout << "Using for kinematic reconstruction TopMASS = " << TopMASS << std::endl;
}
...
void KinematicReconstruction::setRandomNumberSeeds(const LV& lepton, const LV& antiLepton, const LV& jet1, const LV& jet2)const
{
    ...
    unsigned int seed = static_cast<int>( 1.e6*(jet1.pt()/jet2.pt()) * std::sin((lepton.Pt() + 2.*antiLepton.pt())*1.e6) );
    //seed = seed / 10;
    ...
    bool isHaveSol(false);
        ...
        for(int sm=0; sm<100; ++sm){
            ...
            TVector3 metV3_sm= -b_sm.Vect()-bbar_sm.Vect()-l_sm.Vect()-al_sm.Vect()-vX_reco;
              ...
              //met_sm.SetXYZM(met.Px(),met.Py(),0,0);


            //met_sm = TLorentzVector(0.,0.,0.,0.);
                //gRandom->SetSeed(42);
...
void KinematicReconstruction::loadData(const TString& fileInput)
{
...
// jet,lepton resolutions; mbl mass; W mass;
    ...
    else if(era_ == Era::run2_13tev_2016_25ns) data_path1.Append("/KinReco_input_2016_Run2BtoH_25ns_80X_v036_dilep_sel_25July2017.root");
    else{
...
KinematicReconstructionScaleFactors::KinematicReconstructionScaleFactors(const Era::Era era,
...
void KinematicReconstructionScaleFactors::prepareSF(const SystematicInternal& systematic)
{
    ...
    if(era_ == Era::run1_8tev) {
        ...
        // SF for 80X (35922pb)
        m_sfNomEE = 0.9975; m_sfNomEMu = 1.0000; m_sfNomMuMu = 0.9938;
        m_sfUncEE = 0.0027; m_sfUncEMu = 0.0011; m_sfUncMuMu = 0.0018;
    }
    else{
